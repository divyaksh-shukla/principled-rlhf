...
class LinearBanditRLHFEnv(gym.Env):
    """
    Gymnasium environment for linear bandit RLHF setting.
    Based on the example in Appendix B.3 (Theorem 3.9 proof).
    """
    
    def __init__(self, d=3, B=2.0, device='cpu'):
        super().__init__()
        ...
        # Define the 4 actions with their features (from Appendix B.3)
        ...
        self.theta_star = torch.tensor([-1., 0.1, 0.9], device=device, dtype=torch.float32)
